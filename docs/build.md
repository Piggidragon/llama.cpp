...
| GGML_CUDA_FA_QUANTS           | `all` or `type_K-type_V` list | q4_0-q4_0;q8_0-q8_0;f16-f16;bf16-bf16 | Select which K/V type combinations to compile the FlashAttention CUDA kernels for. `all` compiles every combination, but compilation takes much longer. Otherwise a `;`-separated list of `type_K-type_V` pairs; f16-f16 is always compiled. Combinations that were not compiled fall back to f16-f16 kernel with a warning. Legal types: f16, bf16, q4_0, q4_1, q5_0, q5_1, q8_0. The pairs `q4_1-q4_1`, `q5_0-q5_0` and `q5_1-q5_1` also compile the quantized-native MMA kernels for that type. |
| GGML_CUDA_FA_ALL_QUANTS       | Boolean                | false   | Deprecated alias for `GGML_CUDA_FA_QUANTS=all`.                                                                                                                                                                                                                                                                                                                               |

For a few measured geometries the MMA FlashAttention kernels read the quantized K/V cache in place instead of casting the visible window to F16, which removes that transient copy. The route is opt-in at runtime with `--flash-attn-native-quants`. It always covers `q4_0` and `q8_0` caches, and `q4_1`, `q5_0` and `q5_1` caches when `GGML_CUDA_FA_QUANTS` selects their `type-type` pair or is `all`. See [quantized-native-flash-attention.md](quantized-native-flash-attention.md).
